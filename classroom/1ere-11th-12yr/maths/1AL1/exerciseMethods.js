...
export function exportedNouvelleVersionWrapper(numeroExercice, numeroQuestion) {
    const signature = [
        this.seed,
        this.sup,
        this.sup2,
...
export function exportedReinit() {
    this.listeQuestions = [];
...
export function exportedApplyNewSeed() {
 ...
 */
export function exportedQuestionJamaisPosee(i, ...args) {
    if (i === 0)
        this.listeArguments = [];
    let argsConcatenes = '';
...
function generateSeed({ includeUpperCase = true, includeNumbers = true, length = 4, startsWithLowerCase = false, } = {}) {
    let a = 10;
